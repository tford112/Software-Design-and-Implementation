...
/*  FILE: gumbo.c 
 *  Inputs: HTML page stored in a char*
 *  Outputs: char* buffer containing all the href links 
...
char url_buf[LENGTH]; 
static void search_for_links(GumboNode*, FILE*); 
 ...
 * can come up with reading the HTML file. Text streams also can fail to handle lines more than 254 chars long.  
 */
char* all_urls(char **html_buf) { 
	GumboOutput* output = gumbo_parse(*html_buf);  				
...
/* We have to parse our HTML-generated wget pages for the relevant "href" links 
 * for our crawler to save into our DICTIONARY. We recursively search through 
 * these links for ever more links. These links are then stored into our store 
 * file which will be read into a char* later within allocateBufferForFile */ 
...
/* we take all our links that our gumbo searched and read from the FILE 
 * into a char* buffer that we can return back to crawler.c in extractURLS. */
